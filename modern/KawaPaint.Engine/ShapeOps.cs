...
{
    public static void DrawRectangle(Surface s, double x0, double y0, double x1, double y1, int radius, ColorBgra color)
    {
        double left = Math.Min(x0, x1), right = Math.Max(x0, x1);
        double top = Math.Min(y0, y1), bottom = Math.Max(y0, y1);

        BrushOps.DrawLine(s, left, top, right, top, radius, color);
        BrushOps.DrawLine(s, right, top, right, bottom, radius, color);
        BrushOps.DrawLine(s, right, bottom, left, bottom, radius, color);
        BrushOps.DrawLine(s, left, bottom, left, top, radius, color);
    }

    public static void DrawEllipse(Surface s, double x0, double y0, double x1, double y1, int radius, ColorBgra color)
    {
        double cx = (x0 + x1) / 2, cy = (y0 + y1) / 2;
        double rx = Math.Abs(x1 - x0) / 2, ry = Math.Abs(y1 - y0) / 2;
        if (rx < 0.5 || ry < 0.5)
        {
            BrushOps.FillDisc(s, (int)Math.Round(cx), (int)Math.Round(cy), radius, color);
            return;
        }
        ...
        for (int i = 1; i <= steps; i++)
        {
            ...
            BrushOps.DrawLine(s, px, py, nx, ny, radius, color);
            px = nx; py = ny;
        }
    }
}
