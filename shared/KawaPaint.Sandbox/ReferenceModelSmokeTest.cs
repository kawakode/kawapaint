...
internal static class ReferenceModelSmokeTest
{
    private const string Pyramid = """
        # square pyramid: the base uses relative indices
        v -1 -1 0
        v  1 -1 0
        v  1  1 0
        v -1  1 0
    ...
    {
        ObjMesh model = ObjMesh.Parse(new StringReader(Pyramid));
        ...
        using var first = ReferenceRenderer.Render(model, 128, 96);
        using var second = ReferenceRenderer.Render(model, 128, 96,
            new ReferenceRenderOptions { YawDegrees = -55, PitchDegrees = 15 });
        ...
        AssertThrows(() => ObjMesh.Parse(new StringReader("v 0 0 0\nf 1 2 3\n")),
